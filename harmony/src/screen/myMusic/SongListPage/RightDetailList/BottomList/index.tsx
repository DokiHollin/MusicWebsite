...
import BottomBox from '../../LeftMainList/BottomBox';
...
class BottomList extends React.Component<BottomListProps, BottomListState> {
  ...
  render() {
    ...
    console.log(this.props.currentDetailList)
    ...
    let propData = toJS(JSON.parse(JSON.stringify(this.props.currentDetailList)));
    
    console.log(propData.data)
    // consider special case for the play list
    if(propData.data){
    ...
    if(Array.isArray(propData) &&  propData !== null){
      propData = propData.map((val: any) => {
        val.key = val.MusicID;
        // val.ar = val.artist_name
        // .map((val: any) => `${val.name ? val.name : ''} `);
        val.operation = (
          <Fragment>
            <TransparentButton>
              <PlayCircleOutlined
